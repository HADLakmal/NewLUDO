...
public class PlayerInfoController : MonoBehaviour
{
	...
	IEnumerator withdrawState()
	{
		...
		WWW www = new WWW (BaseURL.base_url + "getWithdrawStatus", form);
		yield return www;
		...
		if (www.error != null) {
			...
			//statistic.transform.GetChild (0).GetComponent<Text> ().text = "HOLD ON";
			if (www.text == "true") {
				statistic.interactable = false;
				statistic.transform.GetChild (0).GetComponent<Text> ().text = "HOLD ON";
			} else {
				statistic.interactable = true;
				statistic.transform.GetChild (0).GetComponent<Text> ().text = "WITHDRAWAL";
			}
				
		}


		WWW wwwAmount = new WWW (BaseURL.base_url + "getUserAmount", form);
		yield return wwwAmount;


		if (wwwAmount.error != null) {
			Debug.Log (wwwAmount.error);

		} else {

			realmoney.text = wwwAmount.text;
		}
