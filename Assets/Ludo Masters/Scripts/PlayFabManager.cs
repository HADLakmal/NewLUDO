...
public class PlayFabManager : Photon.PunBehaviour, IChatClientListener
{
	...
	public void addUser(string id ,string name){
		BaseURL.urlCall = true;
		StartCoroutine (requestAdd (id, name));
	}
	IEnumerator requestAdd(string id,string name)
	{
    ...
    public void Login()
    {
        ...
        PlayFabClientAPI.LoginWithCustomID(request, (result) =>
        {
            PlayFabId = result.PlayFabId;
            Debug.Log("Got PlayFabID: " + PlayFabId);
				PlayerPrefs.SetString("unique_id", PlayFabId);
            Dictionary<string, string> data = new Dictionary<string, string>();
            ...
            if (result.NewlyCreated)
            {
				...
				addUser(PlayFabId,name);
					//wait untill user add
					print(BaseURL.urlCall);
                //addCoinsRequest(StaticStrings.initCoinsCount);
            }
            else
            {
					addUser(PlayFabId,"Guest");
					print(BaseURL.urlCall);
					//wait untill user add
            ...
            GameManager.Instance.nameMy = this.name;
